...
class CCityElement extends CUserTypeString
{
    ...
    public static function GetPublicView($arUserField, $arAdditionalParameters = array())
    {
        ...
        $region = \Dmitry\Edu8\RegionTable::getById($regionId);
        if ($region)
        {
            $regionData = $region->fetch();
            $result .= "<strong>{$regionData['NAME']}</strong>: ";

            $city = \Dmitry\Edu8\CityTable::getById($regionCity);
            if ($city)
            {
                $cityData = $city->fetch();
    ...
    {
        $result = '<select onChange="BX.Edu8.field.changeRegion(this);">';

        $optionsResult = \Dmitry\Edu8\RegionTable::getList([]);
    ...
    {
        $result = '<select onChange="BX.Edu8.field.changeCity(this);">';

        $optionsResult = \Dmitry\Edu8\CityTable::getList([]);
        while ($option = $optionsResult->fetch())
        {
            $optionHTML = '<option data-region-id="' . intval($option['REGION_ID']) . '" value="' .
                intval($option['ID']) . '"';
